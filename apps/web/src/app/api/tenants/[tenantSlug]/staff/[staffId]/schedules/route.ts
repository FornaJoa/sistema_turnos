...
async function assertCanManageStaffSchedules(tenantSlug: string, staffId: string) {
  ...
  if (!isAdmin && !isOwnProfile) {
    return { error: "No tenés permiso para editar estos horarios.", status: 403 as const };
  }
...
export async function GET(
  ...
  try {
    ...
    const access = await assertCanManageStaffSchedules(tenantSlug, staffId);
    if ("error" in access) {
      return NextResponse.json({ error: access.error }, { status: access.status });
    }

    const schedules = await getStaffSchedules(access.tenant.id, staffId);
    return NextResponse.json({ schedules });
  } catch (error) {
    return handleRouteError(error, "staff/schedules/get");
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const parsed = schedulesPatchSchema.safeParse(body);
    if (!parsed.success) {
      return jsonError(parsed.error.errors[0]?.message ?? "Datos inválidos.", 400);
    }

    try {
      const schedules = await replaceStaffSchedules(
        access.tenant.id,
        staffId,
        parsed.data.windows
      );
      ...
      if (error instanceof Error && error.message.includes("Invalid")) {
        return jsonError("Horarios inválidos.", 400);
      }
      throw error;
    }
  } catch (error) {
    return handleRouteError(error, "staff/schedules/put");
  }
}
