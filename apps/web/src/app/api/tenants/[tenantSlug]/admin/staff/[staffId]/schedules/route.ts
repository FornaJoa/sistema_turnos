...
export async function GET(
  ...
  try {
    ...
    const schedules = await getStaffSchedules(tenant.id, staffId);
    return NextResponse.json({ schedules });
  } catch (error) {
    return handleRouteError(error, "admin/staff/schedules/get");
  }
}
...
export async function PUT(
  ...
  try {
    ...
    try {
      const schedules = await replaceStaffSchedules(tenant.id, staffId, parsed.data.windows);
    ...
    return handleRouteError(error, "admin/staff/schedules/put");
  }
}
